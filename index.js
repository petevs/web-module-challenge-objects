...
burger.discount = function discount(role) {
  if (role === "public") {
...
function getLastReview(reviews) {
  ...
  return `${latest.name} gave the restaurant a ${latest.rating} star rating and their feedback was: \"${latest.feedback}\"`;
}
...
function getReviewByRating(reviews, score) {
  ...
  for (let i = 0; i < reviews.length; i++) {
    if (reviews[i].rating >= score) {
      ratingMatches.push(reviews[i]);
    }
  }
...
function getLongReviews(reviews) {
  ...
  let longReviews = [];
  for (let i = 0; i < reviews.length; i++) {
    let wordList = reviews[i].feedback.split(" ");
